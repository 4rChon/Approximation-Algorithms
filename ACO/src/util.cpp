    ...
    if(tsp.is_open())
    {
        while(getline(tsp, line))
        {
            if(line.compare("EDGE_WEIGHT_TYPE") > 1)
                if(!(line.compare("EDGE_WEIGHT_TYPE: EUC_2D") == 0) && !(line.compare("EDGE_WEIGHT_TYPE : EUC_2D") == 0))
                {
...
City parse_line(std::string line)
{
    ...
    return City(subStringToFloat(str_id, str_id.size(), 0)-1,
                subStringToFloat(str_x, str_x.size(), 0),
